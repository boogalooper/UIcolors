...
if (!playbackParameters.count) {
  ...
  if (source.content) {
    ...
    {
      presets.getPresets();
      var w = new Window('dialog', 'UI Colors v.' + ver + ': ' + function () { var i = 0; for (var a in colors) i++; return i }() + ' items loaded'),
        ...
        gButtons = w.add("group{orientation: 'row', alignChildren: ['center', 'center']}"),
        bnOk = gButtons.add('button', undefined, 'Save UIColors.txt', { name: 'ok' }),
        bnCancel = gButtons.add('button', undefined, 'Cancel', { name: 'cancel' });
      ...
      w.onShow = function () {
        ...
        colorThemeVisiblity(colorBars)
        loadPresets()
        if (dlPreset.items.length == 3 && cfg.firstRun) {
          if (confirm('It looks like this is the first run of the script.\nDo you want to save a copy of the original UIColors.txt file before continuing?')) {
            presets.putPreset('original UIColors.txt backup', colors, 'add')
            loadPresets();
          }
          cfg.firstRun = false
          cfg.putScriptSettings(UUID[0])
